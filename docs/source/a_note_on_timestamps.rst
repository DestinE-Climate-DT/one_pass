...
=======
- option 2 : the new time stamp is greater than the old time stamp by :math:`<= 2x` the time step. This will return the phrase: ``'Time gap at str(time_stamp) too large, there seems to be data missing, small enough to carry on'`` and will continue with the calculation.

- option 3 : the new time stamp is greater than the old time stamp by :math:`> 2x` the time step. This will cause the Opa to exit with Value Error : ``'Time gap at str(time_stamp) too large, there seems to be some data missing'``.

- option 4 : the new stamp is further back in time than the old time stamp from the checkpoint file. This indicates that the model has crashed and has been re-started or the calculation is starting again. If this new time stamp then corresponds to the beginning of the requested statistic, the Opa will start the calculation again and overwrite the checkpoint file. If this new time stamp does not correspond to the start of the requested statistic, but rather part way through, the Opa will simply skip the data and wait for the data that corresponds to the next time stamp in the sequence. **In this way, the Opa does not require to be manually re-set**.
