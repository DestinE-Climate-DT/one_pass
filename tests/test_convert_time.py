...
@pytest.mark.parametrize("frequency, time_stamp, expected", [
...
def test_convert_time_monthly_missing_time_stamp_input():
    ...
    with pytest.raises(ValueError, match=".*You must provide a time_stamp_input for saving frequency*"):
        convert_time("monthly", None)



@pytest.mark.parametrize("days_in_month, time_stamp_input, expected", [
